...
map<string,RooAddPdf*> getGlobePdfs(RooWorkspace *work, int ncats){
  ...
  for (int cat=0; cat<ncats; cat++){
    //std::cout << Form("sigpdfrelcat%d_allProcs",cat) << " " << Form("cat%d",cat) << std::endl;
    result.insert(pair<string,RooAddPdf*>(Form("cat%d",cat),(RooAddPdf*)work->pdf(Form("sigpdfrelcat%d_allProcs",cat))));
    //std::cout << (RooAddPdf*)work->pdf(Form("sigpdfrelcat%d_allProcs",cat)) << std::endl;
  }
...
pair<double,double> bkgEvPerGeV(RooWorkspace *work, int m_hyp, int cat, pair<double,double> &bkgTotal, bool is2011){
  ...
  if (work->GetName()==TString("cms_hgg_workspace")) {
    ...
    RooAbsPdf *pdf;
    if (is2011) 
      pdf = (RooAbsPdf*)work->pdf(Form("pdf_data_pol_model_7TeV_cat%d",cat));
    else 
      pdf = (RooAbsPdf*)work->pdf(Form("pdf_data_pol_model_8TeV_cat%d",cat));
...
double sobInFWHM(RooWorkspace *sigWS, RooWorkspace *bkgWS, int m_hyp, int cat, double fwhm, pair<double,double> &sobInFWHMTotal, bool is2011){
...
double sobInEffSigma(RooWorkspace *sigWS, RooWorkspace *bkgWS, int m_hyp, int cat, double effSigma, pair<double,double> &sobInEffSigmaTotal, bool is2011){
  ...
  RooDataSet *ggh = (RooDataSet*)sigWS->data(Form("sig_mass_m%d_cat%d",m_hyp,cat));
  //RooDataSet *vbf = (RooDataSet*)sigWS->data(Form("sig_vbf_mass_m%d_cat%d",m_hyp,cat));
  //RooDataSet *tth = (RooDataSet*)sigWS->data(Form("sig_tth_mass_m%d_cat%d",m_hyp,cat));
...
vector<double> sigEvents(RooWorkspace *work, int m_hyp, int nbins, int cat, string binnedSigFileName, vector<double> &sigTotal) {
  ...
  vector<double> result;
  RooDataSet* ds[nbins+1];
  //ds[0] = (RooDataSet*)tempWork->data(Form("sig_mass_m%d_cat%d",m_hyp,cat));
  //double total = ds[0]->sumEntries();
  //sigTotal[0] += total;
  double total = 0;
  result.push_back(total);
 
  for (int i=0; i<nbins; i++) {
    std::cout << Form("sig_Bin%d_mass_m%d_cat%d", i, m_hyp, cat) << std::endl;
    ds[i+1] = (RooDataSet*)tempWork->data(Form("sig_Bin%d_mass_m%d_cat%d", i, m_hyp, cat));
    result.push_back(ds[i+1]->sumEntries());
    sigTotal[i+1] += ds[i+1]->sumEntries();
    sigTotal[0] += ds[i+1]->sumEntries();
    total += ds[i+1]->sumEntries();
    std::cout << "sigTotal[" << i+1 << "] " << cat << " " << sigTotal[i+1] << std::endl;
    std::cout << "result[" << i+1 << "] " << cat << " " << result[i+1] << std::endl;
  }
...
void makeSignalCompositionPlot(int nbins, int nCats, map<string,string> labels, map<string,vector<double> > sigVals, map<string,double> sigEffs, map<string,double> fwhms, map<string,double> sobVals, string outfname, int mh, bool doBkgAndData){

  TString catName[nCats+1];
  for (int cat=0; cat<nCats; cat++) 
    catName[cat] = labels[Form("cat%d",cat)];
  ...
  for (int c=0; c<nCats+1; c++) {
    ...
    if (cat==nCats) 
      pavetext->AddText(Form("%.1f total expected signal",sigVals["all"][0])); 
    else 
      pavetext->AddText(Form("%.1f total expected signal",sigVals[Form("cat%d",cat)][0]));
    ...
    for (int j=0; j<nbins; j++){
      if (cat==nCats) 
	xbinmax += 100.*sigVals["all"][j+1]/sigVals["all"][0];
      else 
	xbinmax += 100.*sigVals[Form("cat%d",cat)][j+1]/sigVals[Form("cat%d",cat)][0];
      pave = new TPave(xbinmin,ybinmin,xbinmax,ybinmax);
      pave->SetFillColor(colors[j]);
      pave->Draw();
      pave->SetBorderSize(0);
      if (cat==nCats) 
	xbinmin += 100.*sigVals["all"][j+1]/sigVals["all"][0];
      else 
	xbinmin += 100.*sigVals[Form("cat%d",cat)][j+1]/sigVals[Form("cat%d",cat)][0];
    }
    pavetext->Draw();
  }
  ...
  float offset = 0.8/nbins;
  float coordX1[6] = {0.20, 0.28, 0.36, 0.44, 0.52, 0.60};
  float coordX2[6] = {0.23, 0.31, 0.39, 0.47, 0.55, 0.63};
  
  for (int i=0; i<nbins; i++) {
    //pave=new TPave(coordX1[i], 0.85, coordX2[i], 0.85+0.03, 0, "NDC");
    pave=new TPave(coordX1[0]+i*offset, 0.85, coordX2[0]+i*offset, 0.85+0.03, 0, "NDC");
    pave->SetFillColor(colors[i]);
    pave->Draw();
    tex_m->DrawLatex(coordX1[0]+0.04+i*offset, 0.84+0.025, Form("Bin%d",i));
  }


  //MATTEO
  int nIncCats=nCats/3;
  std::vector<TLine*> lines;
  for (int i=0; i<nIncCats; i++) {
    lines.push_back(new TLine(0,i*nIncCats+0.5,100,i*nIncCats+0.5));
  ...
  for (int i=0; i<nIncCats; i++) {
    lines.push_back(new TLine(0,i*nIncCats+0.5,3.,i*nIncCats+0.5));
    lines.back()->SetLineColor(kBlack);
    lines.back()->SetLineWidth(2);
    lines.back()->SetLineStyle(2);
    lines.back()->Draw();
  ...
  if (doBkgAndData) {
    ...
    for (int i=0; i<nIncCats; i++) {
      TLine *line = new TLine(0,i*nIncCats+0.5,sobMax,i*nIncCats+0.5);
...
// main function
void makeParametricSignalModelPlots(string sigFitFileName, string outPathName, int m_hyp=125, bool blind=true, bool doTable=false, string bkgdatFileName="0", int nbins=3, int ncats=9, string binnedSigFileName="", bool doCrossCheck=false, bool doMIT=false, bool rejig=false){

  std::string spinProc = "";
  bool splitVH=false, isMassFac, is2011=false;
  //int raiseVHdijet=0;
  ...
  // CHE E`?
  if (rejig) 
    newFileName = runQuickRejig(sigFitFileName,ncats);
  else 
    newFileName = sigFitFileName;

  std::cout << newFileName << std::endl;
  TFile *hggFile = TFile::Open(newFileName.c_str());

  getConfigFromFile(hggFile,is2011,splitVH,isMassFac);
  splitVH = 0; // FIX in workspace
  ...
  for (int i=0; i<ncats; i++) {
    char a[125];
    sprintf(a, "cat%d", i);
    labels.insert(pair<string,string>(a, a));
  }
  ...
  if (doTable) {
    ...
    //int nprocs=nbins;
    //if (splitVH) nprocs+=1;
    for (int i=0; i<nbins+1; i++) sigTotal.push_back(0.);
    for (int cat=0; cat<ncats; cat++){
      if (doBkgAndData) {
        ...
        sobVals.insert(pair<string,double>(Form("cat%d",cat),sobInEffSigma(hggWS,bkgWS,m_hyp,cat,sigEffs[Form("cat%d",cat)],sobTotal,is2011)));
      }
      sigVals.insert(pair<string,vector<double> >(Form("cat%d",cat),sigEvents(hggWS,m_hyp,nbins,cat,binnedSigFileName,sigTotal)));
    }
    ...
    fprintf(nfile,"---------------------------------------------------------------------------------------------------------------\n");
    //fprintf(nfile,"Cat              SigY    ggh    vbf    wh     zh     tth   sEff  FWHM  FWHM/2.35  BkgEv/GeV    Data  DataEv/GeV\n");
    fprintf(nfile,"Cat               SigY");
    for (int i=0; i<nbins; i++) 
      fprintf(nfile,Form("    Bin%d", i));
    fprintf(nfile, "  sEff  FWHM  FWHM/2.35   BkgEv/GeV   Data DataEv/GeV\n");
    fprintf(nfile,"---------------------------------------------------------------------------------------------------------------\n");
	...
	catString = Form("cat%d",cat);
      ...
      for (int i=0; i<nbins; i++)
	fprintf(nfile,"%5.1f%%  ",100.*sigs[i+1]/sigs[0]);
      ...
      for (int i=0; i<nbins; i++)
        fprintf(file,"&  %5.1f\\%%  ",100.*sigs[i+1]/sigs[0]);
    ...
    makeSignalCompositionPlot(nbins,ncats,labels,sigVals,sigEffs,fwhms,sobVals,Form("%s/signalComposition",outPathName.c_str()),m_hyp,doBkgAndData);
...
//{
//  makeParametricSignalModelPlots("../../SimultaneousSignalFitting/CMS-HGG_sigfit_CosThetaStar.root", "pippo", 125, true, true, "../../BackgroundProfileFitting/CMS-HGG_multipdf_CosThetaStar.root", 18);
//}
